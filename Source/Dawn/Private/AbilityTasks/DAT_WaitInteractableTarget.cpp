...
#include "AbilityTasks/DAT_WaitInteractableTarget.h"
#include "Character/DPlayerCharacter.h"
#include "Dawn/Dawn.h"
#include "Interfaces/DInteractable.h"

UDAT_WaitInteractableTarget::UDAT_WaitInteractableTarget(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	bTraceAffectsAimPitch = true;
}

UDAT_WaitInteractableTarget* UDAT_WaitInteractableTarget::WaitForInteractableTarget(UGameplayAbility* OwningAbility,
	FName TaskInstanceName, FCollisionProfileName TraceProfile, float MaxRange, float TimerPeriod, bool bShowDebug)
{
	UDAT_WaitInteractableTarget* MyObj = NewAbilityTask<UDAT_WaitInteractableTarget>(OwningAbility, TaskInstanceName);		//Register for task list here, providing a given FName as a key
	MyObj->TraceProfile = TraceProfile;
	MyObj->MaxRange = MaxRange;
	MyObj->TimerPeriod = TimerPeriod;
	MyObj->bShowDebug = bShowDebug;
	
	ADPlayerCharacter* PlayerCharacter = Cast<ADPlayerCharacter>(OwningAbility->GetCurrentActorInfo()->AvatarActor);
	...
	MyObj->StartLocation.SourceComponent = PlayerCharacter->GetMesh();
	MyObj->StartLocation.SourceSocketName = PlayerCharacter->GetSocketNameTraceSource();
	MyObj->StartLocation.SourceAbility = OwningAbility;

	return MyObj;
}

void UDAT_WaitInteractableTarget::Activate()
{
	UWorld* World = GetWorld();
	World->GetTimerManager().SetTimer(TraceTimerHandle, this, &UDAT_WaitInteractableTarget::PerformTrace, TimerPeriod, true);
}

void UDAT_WaitInteractableTarget::OnDestroy(bool AbilityEnded)
{
	UWorld* World = GetWorld();
...
void UDAT_WaitInteractableTarget::LineTrace(FHitResult& OutHitResult, const UWorld* World, const FVector& Start,
	const FVector& End, FName ProfileName, const FCollisionQueryParams Params, bool bLookForInteractableActor) const
{
	check(World);
		...
		if (!Hit.GetActor() || Hit.GetActor() != Ability->GetCurrentActorInfo()->AvatarActor.Get())
		{
			...
			if (bLookForInteractableActor && Hit.GetActor())
			{
					...
					== ECollisionResponse::ECR_Overlap)
				{
					...
					bool bIsInteractable = Hit.GetActor()->Implements<UDInteractable>();

					if (bIsInteractable && IDInteractable::Execute_IsAvailableForInteraction(Hit.GetActor(), Hit.Component.Get()))
					{
						OutHitResult = Hit;
...
void UDAT_WaitInteractableTarget::AimWithPlayerController(const AActor* InSourceActor, FCollisionQueryParams Params,
...
bool UDAT_WaitInteractableTarget::ClipCameraRayToAbilityRange(FVector CameraLocation, FVector CameraDirection,
...
void UDAT_WaitInteractableTarget::PerformTrace()
{
...
FGameplayAbilityTargetDataHandle UDAT_WaitInteractableTarget::MakeTargetData(const FHitResult& HitResult) const
{
